...
1. Obter texto fiel do SGD quando o JSON ODBC estiver incompleto: use **`scripts/Consultar-PSAI-SGD.ps1`** — pede **sempre** o seu utilizador e senha do SGD (o `.env` não fornece credenciais). Ex.: `.\scripts\Consultar-PSAI-SGD.ps1 130298 --json`. As saídas ficam em **`projeto-filho/data/sgd-psai-consultas/`** (consultas, arquivo HTML/grids, logs, sessão). Credenciais opcionais: `.sgd-credentials.local` nessa mesma pasta — ver `data/sgd-psai-consultas/README.md`. Na raiz do General existe o script equivalente em `scripts/`. Ver `banco-dados/sais/cache/REFERENCIA.md`.
