...
# dominios impactados (slugs Escrita, mesmo criterio que gerar-indices-sais.ps1
# via banco-dados/config/modulos-keywords.json) e gravidade.
#
# USO:
#   .\gerar-indices-enriquecidos.ps1
...
# Raiz do projeto General (pasta que contem banco-dados/, scripts/)
...
function Extrair-ModulosImpactados {
    param(
        [string]$Descricao,
        [hashtable]$ModuloKeywords
    )

    # Classificacao multi-dominio alinhada a gerar-indices-sais.ps1 (modulos-keywords.json).
    if (-not $Descricao) { return @("nao-classificado") }
    ...
    if ($slugs.Count -eq 0) { return @("nao-classificado") }
    return $slugs
}
...
Write-Host "  |  Gerador de Indices Enriquecidos v2.5.0   |" -ForegroundColor Cyan
Write-Host "  |  Admin - Escrita                           |" -ForegroundColor Cyan
Write-Host "  =============================================" -ForegroundColor Cyan
Write-Host ""
...
# Dominios Escrita (mesmo JSON que gerar-indices-sais.ps1)
...
if (Test-Path $kwFile) {
    ...
    Write-Host "  Dominios: $($moduloKeywords.Count) (modulos-keywords.json)" -ForegroundColor DarkGray
} else {
    Write-Host "  AVISO: modulos-keywords.json nao encontrado, usando fallback Escrita" -ForegroundColor DarkYellow
    $moduloKeywords = @{
...
$moduloNomes["nao-classificado"] = "Nao classificado"
Write-Host ""
...
foreach ($reg in $todosRegistros) {
    ...
    $modulos = Extrair-ModulosImpactados -Descricao $descOriginal -ModuloKeywords $moduloKeywords
...
# 4c: Indices por dominio Escrita (slug = sais/indices/modulos/{slug}.md)
Write-Info "Gerando indices por dominio (por-modulo)..."
...
foreach ($modEntry in $todosModulos.GetEnumerator()) {
    $dominioSlug = $modEntry.Key
    $modPath = Join-Path $modulosDir "$dominioSlug.json"
    $nomeExib = if ($moduloNomes.ContainsKey($dominioSlug)) { $moduloNomes[$dominioSlug] } else { $dominioSlug }

    $saidaMod = @{
        gerado_em       = (Get-Date -Format "yyyy-MM-ddTHH:mm:ss")
        dominio_slug    = $dominioSlug
        modulo          = $nomeExib
        total_registros = $modEntry.Value.Count
        dados           = @($modEntry.Value)
    }
    $null = Smart-Write $modPath ($saidaMod | ConvertTo-Json -Depth 4)
    Write-Host "  Dominio $dominioSlug ($nomeExib): $($modEntry.Value.Count) registros" -ForegroundColor DarkGray
}
Write-OK "$($todosModulos.Count) indices por dominio gerados (por-modulo/)"
...
Write-Host "    - Por dominio: $($todosModulos.Count) slugs" -ForegroundColor DarkGray
