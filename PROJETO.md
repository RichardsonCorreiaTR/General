...
| 2.22 | 2026-06-10 | Agente IA + Gerente | **v2.4.39**: GitHub-only no projeto-filho. Analista NAO clona mais o codigo. Removidos `scripts/atualizar-codigo.ps1`, `scripts/atualizar-codigo-fonte.ps1` e `config/codigo-fonte.json` do filho; campo `codigo_local` removido de `config/caminhos.json`; `agente-codigo.mdc` reescrito (somente `gh api`); `acesso-github.mdc` enfatiza unico modo; `verificar-ambiente.ps1` sem checagem de codigo local; `instalar-projeto-filho.ps1` (General) com `Test-GitHubAccess` no lugar do antigo `Install-GitCode`/`Install-FromZip`; docs (SETUP, SETUP-GITHUB, PROMPT-INSTALACAO, PROJETO, GUIA-RAPIDO, CORRECAO-SYMLINKS) atualizadas. General mantem Modo A (gestor tem clone local em `C:\1 - A\B\Programas\brtap-dominio`). |
